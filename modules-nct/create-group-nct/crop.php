    ...
    if ($errorCode === UPLOAD_ERR_OK) {
      ...
      //  delete_directory($path);
        ...
        if ($type == IMAGETYPE_GIF || $type == IMAGETYPE_JPEG || $type == IMAGETYPE_PNG) {
              $group_logo_storage = new storage();
              if (!extension_loaded('imagick')) {
                  echo "imagick not installed...";
              } else {
                  $filename = $img_name;
                  $tempname = $file['tmp_name'];
                  $main_img = $group_logo_storage->upload_object('av8db','',$tempname,$temp_src);
                  $get_main_img = $group_logo_storage->getImageUrl1('av8db',$img_name,$temp_src2);
              }
              if ($get_main_img) {
...
$crop_img = $cropped_image->upload_objectBlob('av8db',$pro_pic_name,$im->getImageBlob(),$temp_src2);
$crop_img_res = $cropped_image->getImageUrl1('av8db',$pro_pic_name,$temp_src2);
