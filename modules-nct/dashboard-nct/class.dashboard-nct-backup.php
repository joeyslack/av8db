...
class Dashboard extends Profile {
    ...
    public function processPostUpdate($posted_or_shared = "p", $type = "u", $status = "p", $shared_feed_id = "") {
        ...
        if (isset($_FILES['feed_image']) && !($_FILES['feed_image']['error'])) {
           ...
           // echo "<pre>";print_r();die;
            ...
            $result = $this->upload_object('av8db',$_FILES['feed_image']['name'],$_FILES['feed_image']['tmp_name']);
       ...
       // $query_without_limit = $query = "SELECT * FROM tbl_feeds WHERE ( " . $connectionsQuery . $followingCompaniesQuery . " shared_with = 'p' ) AND user_id != '" . $user_id . "' AND status = 'p' AND group_id IS NULL ORDER BY id DESC ";
        ...
        if ($feeds) {
            ...
            for ($i = 0; $i < count($feeds); $i++) {
                $con = getSingleFeed($feeds[$i]['id'],$platform,$user_id);
                $feeds_li .= $con;
                $app_array[] = $con;
            }
    ...
    public function getDashboardPageContent() {
        ...
        $publish_post_url = SITE_URL . "publish-post";
        ...
        $cover_photo = getTableValue('tbl_users','cover_photo',array('id'=>$this->user_id));
        $user_cover = $storage2->getImageUrl1('av8db','th1_'.$cover_photo,$src2);
        // $img=getImageURL("user_profile_picture", $this->session_user_id, "th4");
        $src3 = "users-nct/".$this->user_id.'/';
        $profile_picture_name = getTableValue('tbl_users','profile_picture_name',array('id'=>$this->user_id));
        $image_url = $storage2->getImageUrl1('av8db','th4_'.$profile_picture_name,$src3);
        $user_name = $this->db->pdoQuery('SELECT CONCAT(first_name," ",last_name) as user_name FROM tbl_users WHERE id = '.$this->user_id.'')->result();
        $is_image = getimagesize($image_url);
        if(!empty($is_image)){
            $img = '<img src="'.$image_url.'" alt="'.$user_name['user_name'].'">';
        }else{
            $img = '<span class="profile-picture-character">'.ucfirst(mb_substr($user_name['user_name'], 0, 1, 'utf-8')).'</span>';
        }
       ...
       // _print_r($job_detail_array);exit();
        ...
        if ($job_detail_array) {
            ...
            foreach ($job_detail_array as $key => $value) {
                ...
                // }
                $job_src = 'company-logos-nct/';
                $company_logo_url = $job_storage->getImageUrl1('av8db','th2_'.$value['company_logo'],$job_src);
    ...
    public function getCompanySuggetions() {
        ...
        $query = "SELECT c.*,i.industry_name_".$this->lId." as industry_name,l.formatted_address,l.country,l.state,l.city1,l.city2,
                ...
                WHERE c.company_industry_id IN ('$industry_ids')
                AND c.user_id != ?
                AND c.status = ?
                AND c.company_type = ?
        ...
        if ($company_detail_array) {
            ...
            foreach ($company_detail_array as $key => $value) {
                ...
                //     //$company_logo_url = '<img src="'.$company_logo_url.'" alt="'.$value['company_name'].'">';
                // }
                $company_src = 'company-logos-nct/';
                $company_logo_url = $company_storage->getImageUrl1('av8db','th2_'.$value['company_logo'],$company_src);
                $is_image = getimagesize($company_logo_url);
                if(!empty($is_image)){
                    $company_logo_url = '<img src="'.$company_logo_url.'" alt="'.$value['company_name'].'">';
                }else{
        ...
        //if ($this->usersCurrentIndustry != "") {
            ...
            if ($groups) {
                ...
                for ($i = 0; $i < count($groups); $i++) {
                    ...
                    $group_image = getTableValue('tbl_groups','group_logo',array('id'=>$groups[$i]['id']));
                    $group_src = 'group-logos-nct/';
                    $group_logo_url = $group_storage->getImageUrl1('av8db','th2_'.$group_image,$group_src);
                    $is_image = getimagesize($group_logo_url);
                    if(!empty($is_image)){
                        $group_logo_url_web = '<img src="'.$group_logo_url.'" alt="'.ucfirst($groups[$i]['group_name'][0]).'">';
                    }else{
                        $group_logo_url_web = '<span src="profile-picture-character">'.ucfirst($groups[$i]['group_name'][0]).'</span>';
                    }
                    ...
                    // $creator_photo = '<a href=' . get_user_profile_url($groups[$i]['creatorId']) . '>' . getUserProfilePictureURL($groups[$i]['creatorId'], "th3") . '</a>';
                    $creator_profile_image = getTableValue('tbl_users','profile_picture_name',array('id'=>$groups[$i]['creatorId']));
                    $creator_src = 'users-nct/'.$groups[$i]['creatorId'];
                    $creator_photo = $group_storage->getImageUrl1('av8db','th3_'.$creator_profile_image,$creator_src);
                    $is_image = getimagesize($creator_photo);
                    if(!empty($is_image)){
                        $creator_photo = '<img src="'.$creator_photo.'" alt="'.ucwords($groups[$i]['first_name']).' '.ucwords($groups[$i]['last_name']).'">';
                    }else{
                        $creator_photo = '<span class="profile-picture-character">'.ucfirst($groups[$i]['first_name']).'</span>';
                    }
                    $fields_replace = array(
    ...
    public function getPeopleYouKnow($currentPage = 1, $main_page = false, $call_from_ajax = false,$platform='web',$app_user_id=0,$keyword='') {
       ...
       $query = "SELECT u.id, CONCAT(u.first_name, ' ', u.last_name) as user_name
        ...
        if ($main_page) {
            ...
            if ($connection_detail_array) {
                require_once(DIR_MOD."storage.php");
                $people_you_may_know_storage = new storage();
                foreach ($connection_detail_array as $key => $value) {
                    $common_connection_html = NULL;
                    ...
                    $user_name = $this->db->pdoQuery("SELECT CONCAT(first_name,' ',last_name) as user_name FROM tbl_users WHERE id = ".$value['id']."")->result();
                    $cover_photo = getTableValue('tbl_users','profile_picture_name',array('id'=>$value['id']));
                    $people_you_may_know_src = 'users-nct/'.$value['id'].'/';
                    $user_cover = $people_you_may_know_storage->getImageUrl1('av8db','th3_'.$cover_photo,$people_you_may_know_src);
                    $is_image = getimagesize($user_cover);
                    if(!empty($is_image)){
                        $user_cover = '<img src="'.$user_cover.'" alt="'.$user_name['user_name'].'">';
                    }else{
                        $user_cover = '<span class="profile-picture-character">'.ucfirst(mb_substr($user_name['user_name'], 0, 1, 'utf-8')).'</span>';
                    }
...
*/
    ...
    public function getConnectionsPageContent($user_id, $currentpage = 1, $call_from_ajax = false, $keyword = '',$platform = 'web') {
        ...
        $fields_replace = array(
            ...
            $this->session_user_id,
            ucwords(filtering($user_info['first_name'], 'output')) . " " .ucwords( filtering($user_info['last_name'], 'output')),
            getImageURL('user_profile_picture',$user_id, "th4"),
            //ucwords(getUserHeadline($user_id)),
            get_user_profile_url($user_id),
            encryptIt(filtering($user_info['id'], 'output', 'int')),
            $removeButton,
    ...
    function postComment($feed_id =0 ,$user_id=0) {
        ...
        if($comment != ''){
            ...
            if ($comment_id) {
                ...
                $response['comment_html'] = getSingleCommentBox($feed_id, $comment_id);
