...
class Search extends Home {
    function __construct($current_user_id=0,$platform='web') {
        ...
        // $getCurrentUsersIndustry = $this->db->pdoQuery("SELECT ue.industry_id FROM tbl_users u LEFT JOIN tbl_user_experiences ue ON ue.user_id = u.id  AND ue.is_current = 'y' LEFT JOIN tbl_companies c ON c.id = ue.company_id WHERE u.email_verified = ? AND u.status = ? AND u.id = ? ",array('y','a',$this->current_user_id))->result();
        $this->usersCurrentIndustry = '';
        // if ($getCurrentUsersIndustry) {
        //     $this->usersCurrentIndustry = $getCurrentUsersIndustry['industry_id'];
        // }
        $this->formatted_address = $this->city1 = '';
    ...
    public function getCompanySizes($currentPage = 1, $filter_type = "") {
        ...
        //         print_r($selectedCompanySizes);exit();
        //     }
    ...
    public function getIndustries($currentPage = 1, $filter_type = "") {
        ...
        if ($industries) {
            ...
            if (isset($_GET['industries']) && !empty($_GET['industries'])) {
                ...
                //echo "sad";print_r($selectedIndustries);exit();
            }
    ...
    public function getGroupType($currentPage = 1, $filter_type = "") {
        $final_result = NULL;
        $limit = 15;
        $next_available_records = 0;
        $offset = ($currentPage - 1 ) * $limit;
        $query = "SELECT id,group_type_".$this->lId." as group_type FROM tbl_group_types WHERE status = ? ";
        $wherearr=array('a');
        $query_with_limit = $query . " LIMIT " . $limit . " OFFSET " . $offset;
        $industries = $this->db->pdoQuery($query_with_limit,$wherearr)->results();
        if ($industries) {
            ...
            $next_records = $this->db->pdoQuery($query_with_next_limit,$wherearr)->results();
            $next_available_records = count($next_records);
            $single_industry_li_filter = new Templater(DIR_TMPL . $this->module . "/single-group-type-li-filter-nct.tpl.php");
            $single_industry_li_filter_parsed = $single_industry_li_filter->parse();
            $fields = array("%FILTER_TYPE%","%GROUP_ID%","%GROUP_TYPE_NAME%","%CHECKED%");
            $selectedIndustries = array();
            if (isset($_GET['grouptype']) && !empty($_GET['grouptype'])) {
                $selectedIndustries = $_GET['grouptype'];
            }
            for ($i = 0; $i < count($industries); $i++) {
                $fields_replace = array(
                    $filter_type,
                    $industries[$i]['id'],
                    filtering($industries[$i]['group_type']),
                    ( ( in_array($industries[$i]['id'], $selectedIndustries) ) ? 'checked="checked"' : '' )
                );
                $final_result .= str_replace($fields, $fields_replace, $single_industry_li_filter_parsed);
            }
    ...
    public function getRating($currentPage = 1, $filter_type = "") {
        $final_result = NULL;
        $limit = 15;
        $next_available_records = 0;
        $offset = ($currentPage - 1 ) * $limit;
        
        $single_industry_li_filter = new Templater(DIR_TMPL . $this->module . "/single-company_rating-li-filter-nct.tpl.php");
        $single_industry_li_filter_parsed = $single_industry_li_filter->parse();
        $fields = array("%FILTER_TYPE%","%RATE_ID%");
        $selectedIndustries = array();
        for ($i = 1; $i <= 5 ; $i++) {
            $fields_replace = array(
                $filter_type,
    ...
    public function getFerryPilotRatings($currentPage = 1, $filter_type = "") {
        $final_result = NULL;
        $limit = 15;
        $next_available_records = 0;
        $offset = ($currentPage - 1 ) * $limit;
        $single_industry_li_filter = new Templater(DIR_TMPL . $this->module . "/single-ferry_pilot_ratings-li-filter-nct.tpl.php");
        $single_industry_li_filter_parsed = $single_industry_li_filter->parse();
        $fields = array("%FILTER_TYPE%","%RATE_ID%");
        $selectedIndustries = array();
        for ($i = 1; $i <= 5 ; $i++) {
    ...
    public function getHomeAirport($currentPage = 1, $filter_type = "") {
        $final_result = NULL;
        $limit = 15;
        $next_available_records = 0;
        $offset = ($currentPage - 1 ) * $limit;
        
        //$query = "SELECT id,airport_name_".$this->lId." as airport_name FROM tbl_airport WHERE status = ? ";
        $entity = filtering($_GET['entity'], "input");
        
        if ($entity == 'users') {
            $selected_airport_id = getTableValue("tbl_user_airports", "id",array('isActive'=> 'y','isAdminApprove' => 'y'));
            if ($selected_airport_id > 0) {
                 $query = "SELECT distinct(a.id),a.airport_identifier as airport_name FROM tbl_user_airports as ua LEFT JOIN tbl_airport as a ON ua.airport_id = a.id WHERE a.status = ? ";               
            }else{
                $query = "SELECT id,airport_identifier as airport_name FROM tbl_airport WHERE status = ? ";    
            }
        }elseif ($entity == 'companies') {
            $selected_airport_id = getTableValue("tbl_companies", "closest_airport_id",array('status'=> 'a','isCompanyEmailVerify' => 'y'));
            if ($selected_airport_id > 0) {
                $query = "SELECT distinct(a.id),a.airport_identifier as airport_name FROM tbl_companies as ua LEFT JOIN tbl_airport as a ON ua.closest_airport_id = a.id WHERE a.status = ? ";            
            }else{
                $query = "SELECT id,airport_identifier as airport_name FROM tbl_airport WHERE status = ? ";
            }    
        }else{
        ...
        $wherearr=array('a');
        $query_with_limit = $query . " LIMIT " . $limit . " OFFSET " . $offset;
        $homeairport = $this->db->pdoQuery($query_with_limit,$wherearr)->results();
        
        if ($homeairport != '') {
            $query_with_next_limit = $query . " LIMIT " . $limit . " OFFSET " . ( $offset + $limit );
            $next_records = $this->db->pdoQuery($query_with_next_limit,$wherearr)->results();
            $next_available_records = count($next_records);
            $single_home_airport_li_filter = new Templater(DIR_TMPL . $this->module . "/single-home-airport-li-filter-nct.tpl.php");
            $single_home_airport_li_filter_parsed = $single_home_airport_li_filter->parse();
            
            $fields = array("%FILTER_TYPE%","%AIRPORT_ID%","%AIRPORT_NAME%","%CHECKED%");
            
            $selectedIndustries = array();
            if (isset($_GET['homeairport']) && !empty($_GET['homeairport'])) {
                $selectedIndustries = $_GET['homeairport'];
            }
            for ($i = 0; $i < count($homeairport); $i++) {
                $fields_replace = array(
                    $filter_type,
                    $homeairport[$i]['id'],
                    filtering($homeairport[$i]['airport_name']),
                    ( ( in_array($homeairport[$i]['id'], $selectedIndustries) ) ? 'checked="checked"' : '' )
                );
                $final_result .= str_replace($fields, $fields_replace, $single_home_airport_li_filter_parsed);
            }
    ...
    public function getCompanyRatingFilter($filter_type = "", $company_rating_filter_hidden) {
        $final_result = NULL;
        
        $company_filters = new Templater(DIR_TMPL . $this->module . "/company_rating_filters-nct.tpl.php");
        $company_filters->set('ratings', $this->getRating(1, $filter_type));
        $company_filters_parsed = $company_filters->parse();
        
        $fields = array("%FILTER_TYPE%","%COMPANY_RATING_FILTER_HIDDEN%");
        
        $fields_replace = array(
            $filter_type,
            ( ( $filter_type == "adv_" ) ? "" : $company_rating_filter_hidden )
        );
        $final_result = str_replace($fields, $fields_replace, $company_filters_parsed);
        return $final_result;
    } 
    public function getFerryPilotRatingsFilter($filter_type = "", $ferry_pilot_ratings_filter_hidden) {
        $final_result = NULL;
        
        $company_filters = new Templater(DIR_TMPL . $this->module . "/ferry_pilot_ratings_filters-nct.tpl.php");
        $company_filters->set('ferry_pilot_ratings', $this->getFerryPilotRatings(1, $filter_type));
        $company_filters_parsed = $company_filters->parse();
        
        $fields = array("%FILTER_TYPE%","%FERRY_PILOT_RATINGS_FILTER_HIDDEN%");
        
        $fields_replace = array(
            $filter_type,
            ( ( $filter_type == "adv_" ) ? "" : $ferry_pilot_ratings_filter_hidden )
        );
        $final_result = str_replace($fields, $fields_replace, $company_filters_parsed);
        return $final_result;
    }
    public function getUserHomeAirportFilter($filter_type = "", $user_home_airport_filter_hidden) {
        $final_result = NULL;
        
        $user_home_airport_filters = new Templater(DIR_TMPL . $this->module . "/user_home_airport_filters-nct.tpl.php");
        $user_home_airport_filters->set('homeairport', $this->getHomeAirport(1, $filter_type));
        $user_home_airport_filters_parsed = $user_home_airport_filters->parse();
        
        $fields = array("%FILTER_TYPE%","%USER_HOME_AIRPORT_FILTER_HIDDEN%");
        
        $fields_replace = array(
            $filter_type,
            ( ( $filter_type == "adv_" ) ? "" : $user_home_airport_filter_hidden )
        );
        $final_result = str_replace($fields, $fields_replace, $user_home_airport_filters_parsed);
        return $final_result;
    }
    ...
    public function getSortingFilter($filter_type = "", $sortings_filter_hidden) {
        ...
        if(isset($_GET['sorting']) && $_GET['sorting'] != '')
        {
            ...
            for ($i = 0; $i < count($sorting); $i++) {
                $checked_lh = ($sorting[$i] == 'lh') ? 'checked' : '';   
                $checked_hl = ($sorting[$i] == 'hl') ? 'checked' : '';   
            }
        }

        $fields_replace = array(
            $filter_type,
            ( ( $filter_type == "adv_" ) ? "" : $sortings_filter_hidden ),
            $checked_lh,
            $checked_hl,
        );
    ...
    public function getFerryPilotUserFilter($filter_type = "", $ferry_pilot_user_filter_hidden) {
        $final_result = NULL;
        $relationship_filters = new Templater(DIR_TMPL . $this->module . "/ferry-pilot-user-filters-nct.tpl.php");
        $relationship_filters_parsed = $relationship_filters->parse();
        $fields = array("%FILTER_TYPE%","%FERRY_PILOT_USER_FILTER_HIDDEN%");
        $fields_replace = array(
            $filter_type,
            ( ( $filter_type == "adv_" ) ? "" : $ferry_pilot_user_filter_hidden )
        );
        $final_result = str_replace($fields, $fields_replace, $relationship_filters_parsed);
        return $final_result;
    }
    public function getGroupTypeFilter($filter_type = "", $group_type_filter_hidden) {
        $final_result = NULL;
        $industry_filters = new Templater(DIR_TMPL . $this->module . "/group-type-filters-nct.tpl.php");
        $industry_filters->set('groups', $this->getGroupType(1, $filter_type));
        $industry_filters_parsed = $industry_filters->parse();
        $fields = array("%FILTER_TYPE%","%GROUP_TYPE_FILTER_HIDDEN%");
        $fields_replace = array(
            $filter_type,
            ( ( $filter_type == "adv_" ) ? "" : $group_type_filter_hidden )
        );
        $final_result = str_replace($fields, $fields_replace, $industry_filters_parsed);
        return $final_result;
    }
    ...
    public function getUsers($current_user_id, $currentPage) {
        ...
        $applied_home_airport_filter_li_tpl = new Templater(DIR_TMPL . $this->module . "/applied-home-airport-filter-li-nct.tpl.php");
        $applied_home_airport_filter_li_tpl_parsed = $applied_home_airport_filter_li_tpl->parse();

        $applied_ferry_pilot_filter_li_tpl = new Templater(DIR_TMPL . $this->module . "/applied-ferry-pilot-filter-li-nct.tpl.php");
        $applied_ferry_pilot_filter_li_tpl_parsed = $applied_ferry_pilot_filter_li_tpl->parse();
        ...
        $query = "SELECT u.id,u.first_name,u.last_name, l.formatted_address 
                    FROM tbl_users u LEFT JOIN tbl_locations l ON l.id = u.location_id " . $userSkillsLeftJoin . " LEFT JOIN tbl_ferry_pilot_rating fr ON u.id = fr.receiver_id LEFT JOIN tbl_user_airports as ua ON u.id = ua.user_id LEFT JOIN tbl_airport as ai ON ua.airport_id = ai.id WHERE u.email_verified = ? AND u.status = ? AND u.id != ?  ";

        $wherearr=array('y','a',$this->current_user_id);
        if (isset($_GET['keyword']) && $_GET['keyword'] != '') {
            $keyword = filtering($_GET['keyword'], 'input');
            $query .= " AND ( u.first_name LIKE '%" . $keyword . "%' OR u.last_name LIKE '%" . $keyword . "%' OR CONCAT( u.first_name, ' ', u.last_name ) LIKE '%" . $keyword . "%' OR u.email_address LIKE '%" . $keyword . "%' ) ";
        }
        ...
        if (isset($_GET['ferry_pilot_user']) && !empty($_GET['ferry_pilot_user'])) {
            $ferry_pilot_user = $_GET['ferry_pilot_user'];
            if ($ferry_pilot_user[0] == 'y')
            {
                $query .= " AND u.isFerryPilot = 'y'";
                
                $fields_replace_applied_filter = array(
                    'Ferry Pilot'
                );
                $applied_filters .= str_replace($fields_applied_filter, $fields_replace_applied_filter, $applied_ferry_pilot_filter_li_tpl_parsed);
            }
        }
        if (isset($_GET['homeairport']) && !empty($_GET['homeairport'])) {
            $homeairport = $_GET['homeairport'];
            $query .= " AND ua.airport_id IN (" . implode(",", $homeairport) . ") ";
            for ($i = 0; $i < count($homeairport); $i++) {
                $airport_identifier = getTableValue("tbl_airport", "airport_identifier", array("id" => $homeairport[$i]));
                $fields_replace_applied_filter = array(
                    $airport_identifier,
                    "adv_airport_name_" . $homeairport[$i]
                );
                $applied_filters .= str_replace($fields_applied_filter, $fields_replace_applied_filter, $applied_home_airport_filter_li_tpl_parsed);
            }
        }
        if (isset($_GET['groups']) && !empty($_GET['groups'])) {
            $groups = $_GET['groups'];
        ...
        //     }
        // }
        if(isset($_GET['ferry_ratings']) && !empty($_GET['ferry_ratings']) && implode(",", $_GET['ferry_ratings']) != ""){
            $ferry_ratings = $_GET['ferry_ratings'];
            $query .= " AND fr.rating IN (" . implode(",", $ferry_ratings) . ") ";
        }
        ...
        $query .="GROUP BY u.id ORDER BY  CASE WHEN u.location_id THEN 1 ELSE 2 END ,u.id ASC";
        ...
        if ($users) {
            ...
            $user_logo_storage1 = new storage();
            ...
            for ($i = 0; $i < count($users); $i++) {
                ...
                $industry_name = $profile_picture_name = $userimage_final = '';
                ...
                $user_profile = DIR_NAME_USERS."/".$user_id."/";
                $get_profile_picture = $this->db->select("tbl_users", "*", array("id" => $user_id))->result();
                ...
                if ($profile_picture_name == '') {
                    $userimage_final = '<span title="' . $get_profile_picture['first_name'] . ' ' . $get_profile_picture['last_name'] . '" class="profile-picture-character">' . ucfirst($get_profile_picture['first_name'][0]) . '</span>';
                }else 
                {   
                    $img_url = $user_logo_storage1->getImageUrl1('av8db','th3_'.$profile_picture_name,$user_profile);
                ...
                $send_inmail_class = '';
                $skill_match = isset($users[$i]['no_of_skills_matched']) ? $users[$i]['no_of_skills_matched'] : ''; 
                $fields_replace = array(
                    $userimage_final,
                    $user_profile_url,
    ...
    public function getJobs($current_user_id, $currentPage) {
        ...
        $applied_sorting_filter_li_tpl = new Templater(DIR_TMPL . $this->module . "/applied-sorting-filter-li-nct.tpl.php");
        $applied_sorting_filter_li_tpl_parsed = $applied_sorting_filter_li_tpl->parse();
        ...
        if(isset($_GET['sorting']) && $_GET['sorting'] != ''){
            ...
            for ($i = 0; $i < count($sorting); $i++) {
                if($sorting[$i] == 'lh'){
                    $query.=" GROUP BY jobs.id ORDER BY jobs.added_on ASC ";
                    $sorting_label = SEARCH_JOB_OLDEST; 
                }else if($sorting[$i] == 'hl'){
                    $query.=" GROUP BY jobs.id ORDER BY jobs.added_on DESC "; 
                    $sorting_label = SEARCH_JOB_RECENT;
                }else{
                ...
                $applied_filters .= str_replace($fields_applied_filter, $fields_replace_applied_filter, $applied_sorting_filter_li_tpl_parsed);
           }
        }else{
            $query.=" GROUP BY jobs.id ORDER BY (jobs.featured_till >= '".date("Y-m-d")."') DESC, total_score DESC, jobs.id DESC ";    
        }
        ...
        if ($jobs) {
            ...
            for ($i = 0; $i < count($jobs); $i++) {
                ...
                require_once(DIR_MOD . 'common_storage.php');
                $search_job_storage = new storage();

                $logo_url = DIR_NAME_COMPANY_LOGOS."/";
                $company_logo_name = getTableValue("tbl_companies", "company_logo", array("id" => $jobs[$i]['company_id']));
                $company_names = getTableValue("tbl_companies", "company_name", array("id" => $jobs[$i]['company_id']));
                $company_logos_url = '';
                $src = $search_job_storage->getImageUrl1('av8db','th2_'.$company_logo_name,$logo_url);
                $ck = getimagesize($src);
                if (empty($ck)) {
                ...
                $company_logo_url = $company_logo_web_url = $company_logos_url;
                // $company_logo_url = $company_logo_web_url = getImageURL("company_logo", $jobs[$i]['company_id'], "th2",$this->platform);
                $company_logo_web_url = $company_logo_web_url;
    ...
    public function getCompanies($current_user_id, $currentPage) {
        ...
        $applied_company_rating_filter_li_tpl = new Templater(DIR_TMPL . $this->module . "/applied-company-rating-filter-li-nct.tpl.php");
        $applied_company_rating_filter_li_tpl_parsed = $applied_company_rating_filter_li_tpl->parse();
        ...
        $query = "SELECT comp.id,comp.company_name,comp.website_of_company,comp.company_description,comp.owner_email_address, i.industry_name_".$this->lId." as industry_name, l.formatted_address, COUNT(DISTINCT cf.id) as no_of_followers, COUNT(crr.id) as totalRatings,
                    ...
                    LEFT JOIN tbl_industries i ON i.id = comp.company_industry_id 
                    LEFT JOIN tbl_company_followers cf ON cf.company_id = comp.id LEFT JOIN tbl_company_rate_review crr ON comp.id = crr.company_id LEFT JOIN tbl_airport a ON comp.closest_airport_id = a.id 
        ...
        if (isset($_GET['ratings']) && !empty($_GET['ratings'])) {
            ...
            $query .= " AND crr.rating IN (" . implode(",", $ratings) . ") ";
            for ($i = 0; $i < count($ratings); $i++) {
                $fields_replace_applied_filter = array(
                    $ratings[$i],
                    "adv_ratings_" . $ratings[$i]
                );
                $applied_filters .= str_replace($fields_applied_filter, $fields_replace_applied_filter, $applied_company_rating_filter_li_tpl_parsed);
            }
        }
        if (isset($_GET['homeairport']) && !empty($_GET['homeairport'])) {
            $homeairport = $_GET['homeairport'];
            $query .= " AND comp.closest_airport_id IN (" . implode(",", $homeairport) . ") ";
            for ($i = 0; $i < count($homeairport); $i++) {
                $industry_name = getTableValue("tbl_airport", "airport_identifier", array("id" => $homeairport[$i]));
                $fields_replace_applied_filter = array(
                    $industry_name,
                    "adv_airport_name_" . $homeairport[$i]
                );
        ...
        // if (isset($_GET['homeairport']) && !empty($_GET['homeairport'])) {
        //     $homeairport = $_GET['homeairport'];
        //     print_r($homeairport);exit();
        //     $query .= " AND comp.company_industry_id IN (" . implode(",", $homeairport) . ") ";
        //     for ($i = 0; $i < count($homeairport); $i++) {
        //         $airport_name = getTableValue("tbl_airport", "airport_name_".$this->lId, array("id" => $homeairport[$i]));
        //         $fields_replace_applied_filter = array(
        //             $airport_name,
        //             "adv_airport_name_" . $homeairport[$i]
        //         );
        ...
        if(isset($_GET['sorting']) && !empty($_GET['sorting'])){
           ...
           for ($i = 0; $i < count($sorting); $i++) {
                if($sorting[$i] == 'lh'){
                    $query .= " GROUP BY comp.id " . $no_of_followers_query . " ORDER BY totalRatings ASC  ";
                    $sorting_label = LBL_SEARCH_LOWEST_RATED_FIRST;
                }else if($sorting[$i] == 'hl'){
                    $query .= " GROUP BY comp.id " . $no_of_followers_query . " ORDER BY totalRatings DESC  ";
                    $sorting_label = LBL_SEARCH_HIGHEST_RATED_FIRST;
                }else{
        ...
        if ($companies) {
            ...
            require_once(DIR_MOD . 'common_storage.php');
            $search_company_storage = new storage();
            ...
            for ($i = 0; $i < count($companies); $i++) {
                ...
                $company_logo_name=getTableValue("tbl_companies","company_logo",array("id"=>$company_id));
                $company_names =getTableValue("tbl_companies", "company_name", array("id" => $company_id));

                $company_logos_url = '';
                $company_logo_url = $company_logo_web_url =  $src = $search_company_storage->getImageUrl1('av8db','th2_'.$company_logo_name,$logo_url);
                $ck = getimagesize($src);
                if (empty($ck)) {
                    $company_logos_url = '<span title="' . $company_names.'" class="profile-picture-character">' . ucfirst($company_names[0]) . '</span>';
                }else 
                ...
                $company_logo_web_url = $company_logos_url;
                ...
                $range_of_no_of_employees = isset($companies[$i]['range_of_no_of_employees']) ? $companies[$i]['range_of_no_of_employees'] : '';
    ...
    public function getGroups($current_user_id, $currentPage) {
        ...
        $applied_filter_li_tpl_parsed = $applied_filter_li_tpl->parse();
        $fields_applied_filter = array("%FILTER_NAME%","%DATA_ID%");
        $query = "SELECT groups.id,groups.user_id,groups.privacy,groups.group_name,groups.accessibility,gtypes.group_type_".$this->lId." as group_type,u.first_name, u.last_name FROM tbl_groups groups 
                    LEFT JOIN tbl_group_types gtypes ON groups.group_type_id = gtypes.id LEFT JOIN tbl_users u ON u.id = groups.user_id 
        ...
        if (isset($_GET['grouptype']) && !empty($_GET['grouptype'])) {
            $grouptypes = $_GET['grouptype'];
            $query .= " AND groups.group_type_id IN (" . implode(",", $grouptypes) . ") ";
            for ($i = 0; $i < count($grouptypes); $i++) {
                $group_type = getTableValue("tbl_group_types", "group_type", array("id" => $grouptypes[$i]));
                $fields_replace_applied_filter = array(
                    $group_type,
                    "adv_group_type_" . $grouptypes[$i]
                );
                $applied_filters .= str_replace($fields_applied_filter, $fields_replace_applied_filter, $applied_filter_li_tpl_parsed);
            }
        }
        $query .= " GROUP BY groups.id ORDER BY groups.id DESC, groups.id DESC ";
        $totalRecords = count($this->db->pdoQuery($query,$wherearr)->results());
        $query_with_limit = $query . " LIMIT " . $limit . " OFFSET " . $offset;
        $groups = $this->db->pdoQuery($query_with_limit,$wherearr)->results();
        if ($groups) {
            ...
            require_once(DIR_MOD . 'common_storage.php');
            $search_group_storage = new storage();
            ...
            for ($i = 0; $i < count($groups); $i++) {
                ...
                $groups_img = '';
                $group_logo_name = getTableValue("tbl_groups", "group_logo", array("id" => $group_id));

                $src = $search_group_storage->getImageUrl1('av8db','th2_'.$group_logo_name,$src2);
                $ck = getimagesize($src);
                if (!empty($ck)) {
                    $groups_img ='<picture>
                                    <source srcset="' . $src . '" type="image/jpg">
                    ...
                    $groups_img = '<span title="'.$groups[$i]['group_name'].'" class="profile-picture-character">' . ucfirst($groups[$i]['group_name'][0]) . '</span>';
                }

                $group_logo_url = $group_logo_url_web = $groups_img;
                // $group_logo_url = $group_logo_url_web = getImageURL("group_logo", $group_id, "th2",$this->platform);

                $group_logo_url_web = $group_logo_url_web;
                ...
                $users_img = '';
                $user_profile = DIR_NAME_USERS."/".$creator_id."/";
                $user_name = getTableValue("tbl_users", "profile_picture_name", array("id" => $creator_id));
                $first_name = getTableValue("tbl_users", "first_name", array("id" => $creator_id));
                $last_name = getTableValue("tbl_users", "last_name", array("id" => $creator_id));

                $user_src = $search_group_storage->getImageUrl1('av8db','th2_'.$user_name,$user_profile);
                $ck1 = getimagesize($user_src);
                if (!empty($ck1)) {
                    $users_img ='<picture>
                                    <source srcset="' . $user_src . '" type="image/jpg">
                                    <img src="' . $user_src . '" class="" alt="img" /> 
                                </picture>';   
                }else{
                    $users_img = '<span title="'.$first_name.' '.$last_name.'" class="profile-picture-character">' . ucfirst($first_name[0]) . '</span>';  
                }
    ...
    public function getPageContent() {
       ...
       // if($this->current_user_id > 0) {
        ...
        $relationship_filter_hidden = $company_size_filter_hidden = $ferry_pilot_user_filter_hidden =  "hidden";

        $adv_company_filter_hidden = $adv_groups_filter_hidden = $industries_filter_hidden = $adv_industries_filter_hidden = $adv_job_categories_filter_hidden = $company_rating_filter_hidden = $adv_company_rating_filter_hidden = $user_home_airport_filter_hidden = $adv_user_home_airport_filter_hidden = $sortings_filter_hidden = $adv_sortings_filter_hidden = $ferry_pilot_ratings_filter_hidden = $adv_ferry_pilot_ratings_filter_hidden = $adv_group_type_filter_hidden = $group_type_filter_hidden ="hidden";

        $employment_type_filter_hidden = $no_of_followers_filter_hidden = "hidden";
        
        switch ($entity) {
            case "users" : {
                $relationship_filter_hidden = $adv_company_filter_hidden = $adv_groups_filter_hidden = $adv_user_home_airport_filter_hidden =
                    $adv_ferry_pilot_ratings_filter_hidden = $ferry_pilot_user_filter_hidden = $adv_industries_filter_hidden =  "";
                if($this->current_user_id > 0) {
            ...
            case "jobs" : {
                $employment_type_filter_hidden = $adv_company_filter_hidden = $adv_industries_filter_hidden = $adv_job_categories_filter_hidden = $adv_user_home_airport_filter_hidden =
                    $sortings_filter_hidden = "";
            ...
            case "companies" : {
                $company_size_filter_hidden = $adv_industries_filter_hidden = $no_of_followers_filter_hidden = $adv_company_rating_filter_hidden = $adv_user_home_airport_filter_hidden =
                    $sortings_filter_hidden = "";
                if($this->current_user_id > 0) {
            ...
            case "groups" : {
                $adv_user_home_airport_filter_hidden = $group_type_filter_hidden = "";
        ...
        $fields = array(
            ...
            "%COMPANY_RATING_FILTER_HIDDEN%",
            "%COMPANY_RATING_FILTER%",
            "%ADV_COMPANY_RATING_FILTER%",

            "%USER_HOME_AIRPORT_FILTER_HIDDEN%",
            "%USER_HOME_AIRPORT_FILTER%",
            "%ADV_USER_HOME_AIRPORT_FILTER%",

            "%SORTINGS_FILTER_HIDDEN%",
            "%SORTINGS_FILTER%",
            "%ADV_SORTINGS_FILTER%",

            "%FERRY_PILOT_RATINGS_FILTER_HIDDEN%",
            "%FERRY_PILOT_RATINGS_FILTER%",
            "%ADV_FERRY_PILOT_RATINGS_FILTER%",

            "%FERRY_PILOT_USER_FILTER_HIDDEN%",
            "%FERRY_PILOT_USER_FILTER%",
            "%ADV_FERRY_PILOT_USER_FILTER%",

            "%GROUP_TYPE_FILTER_HIDDEN%",
            "%GROUP_TYPE_FILTER%",
            "%ADV_GROUP_TYPE_FILTER%",
        );
        $keyword = "";
        if (isset($_GET['keyword']) && $_GET['keyword'] != "") {
            $keyword = filtering($_GET['keyword']);
        ...
        if($this->current_user_id > 0) {
            $fields_replace = array(
                ...
                $company_li_active_class,
                $group_li_active_class,
                $adv_company_rating_filter_hidden,
                $this->getCompanyRatingFilter("", $company_rating_filter_hidden),
                $this->getCompanyRatingFilter("adv_", $adv_company_rating_filter_hidden),

                $adv_user_home_airport_filter_hidden,
                $this->getUserHomeAirportFilter("", $user_home_airport_filter_hidden),
                $this->getUserHomeAirportFilter("adv_", $adv_user_home_airport_filter_hidden),

                $sortings_filter_hidden,
                $this->getSortingFilter("", $sortings_filter_hidden),
                $this->getSortingFilter("adv_", $sortings_filter_hidden),

                $adv_ferry_pilot_ratings_filter_hidden,
                $this->getFerryPilotRatingsFilter("", $ferry_pilot_ratings_filter_hidden),
                $this->getFerryPilotRatingsFilter("adv_", $adv_ferry_pilot_ratings_filter_hidden),

                $ferry_pilot_user_filter_hidden,
                $this->getFerryPilotUserFilter("", $ferry_pilot_user_filter_hidden),
                $this->getFerryPilotUserFilter("adv_", $ferry_pilot_user_filter_hidden),

                $group_type_filter_hidden,
                $this->getGroupTypeFilter("", $group_type_filter_hidden),
                $this->getGroupTypeFilter("adv_", $group_type_filter_hidden),
            );
        } else {
            $fields_replace = array(
                '',
    ...
    public function getUsersBeforeLogin($currentPage) {
        ...
        $query = "SELECT u.id,u.first_name,u.last_name, l.formatted_address
                    FROM tbl_users u LEFT JOIN tbl_locations l ON l.id = u.location_id 
        ...
        if ($users) {
            ...
            require_once(DIR_MOD . 'common_storage.php');
            $user_logo_storage2 = new storage();
            ...
            for ($i = 0; $i < count($users); $i++) {
                ...
                // if($skills=='')
                //     $class_skill_hide="hidden";
                $class_location= $profile_picture_name = $userimage_final = "";
                ...
                {   
                    $img_url = $user_logo_storage2->getImageUrl1('av8db','th3_'.$profile_picture_name,$user_profile);
                    $up = getimagesize($img_url);
                    if (!empty($up)) {
                        $userimage_final ='<picture>
                                        <source srcset="' . $img_url . '" type="image/jpg">
    ...
    public function getCompaniesBeforeLogin($currentPage) {
        ...
        $query = "SELECT comp.*, i.industry_name, l.formatted_address, COUNT(DISTINCT cf.id) as no_of_followers, COUNT(crr.id) as totalRatings 
                    FROM tbl_companies comp 
                    LEFT join tbl_company_locations cl ON (cl.company_id = comp.id AND cl.is_hq = 'y') 
                    LEFT JOIN tbl_locations l ON l.id = cl.location_id 
                    LEFT JOIN tbl_industries i ON i.id = comp.company_industry_id
                    LEFT JOIN tbl_company_followers cf ON cf.company_id = comp.id
                    LEFT JOIN tbl_company_rate_review crr ON comp.id = crr.company_id LEFT JOIN tbl_airport a ON comp.closest_airport_id = a.id
        ...
        require_once(DIR_MOD . 'common_storage.php');
        $search_company_storage1 = new storage();
