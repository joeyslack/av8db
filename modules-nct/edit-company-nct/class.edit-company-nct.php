<?php 
class Edit_company extends Home {
    ...
    public function uploadCompanyLogo(){
        ...
        $company_logo_temp = new storage();
        if($images_str!=""){             
            if($images_str !="" || $images_str !=NULL)
            {
                $uploadDir = DIR_NAME_COMPANY_LOGOS.'/';
                $th_arr = array();        
                ...
                for ($i = 0; $i < $length; $i++) {
                    ...
                    $resize_img = $company_logo_temp->upload_objectBlob('av8db','th'.($i+1).'_'.$images_str,$im1->getImageBlob(),$uploadDir);
                }
                $im1->clear();
                $im1->destroy();
            ...
            $del = $company_logo_temp->delete_object('av8db',$images_str,'');
            $_SESSION['temp_files']='';
        }   
        return $user_logo;     
    }
    ...
    public function uploadCompanyBanner(){
        ...
        $main_url=$_SESSION['slider_main_url'];
        require_once(DIR_MOD . 'common_storage.php');
        $company_banner_temp = new storage();
        if($images_str!=""){
            if($images_str !="" || $images_str !=NULL){
             $uploadDir = DIR_NAME_COMPANY_BANNER_IMAGES.'/';
            ...
            for ($i = 0; $i < $length; $i++) {
                ...
                $resize_img = $company_banner_temp->upload_objectBlob('av8db','th'.($i+1).'_'.$images_str,$im1->getImageBlob(),$uploadDir);
                $im1->clear();
                $im1->destroy();
            }
            $image_name = $images_str;
            ...
            $del = $company_banner_temp->delete_object('av8db',$images_str,'');
    ...
    public function updateCompanyDetails($company_id,$platform='web') {
        ...
        require_once(DIR_MOD.'common_storage.php');
        $company_img_del_storage = new storage();
        ...
        if (isset($_FILES['company_logo']) && !($_FILES['company_logo']['error']) && $platform == 'app') {
            ...
            if ($_POST['is_logo_removed'] == "true") {
                ...
                if($old_logo['company_logo'] != ''){
                    ...
                    $src2 = 'company-logos-nct/';
                    if ($old_logo['company_logo'] != '') {
                      $main_img = $company_img_del_storage->getImageUrl1('av8db',$old_logo['company_logo'],$src2);
                      $is_main_img = getimagesize($main_img);
                      if(!empty($is_main_img)){
                        $del = $company_img_del_storage->delete_object1('av8db',$old_logo['company_logo'],'',$src2);
                      }
                      $main_img_one = $company_img_del_storage->getImageUrl1('av8db','th1_'.$old_logo['company_logo'],$src2);
                      $is_main_img_one = getimagesize($main_img_one);
                      if(!empty($is_main_img_one)){
                        $del1 = $company_img_del_storage->delete_object1('av8db','th1_'.$old_logo['company_logo'],'',$src2);
                      }
                      $main_img_two = $company_img_del_storage->getImageUrl1('av8db','th2_'.$old_logo['company_logo'],$src2);
                      $is_main_img_two = getimagesize($main_img_two);
                      if(!empty($is_main_img_two)){
                        $del2 = $company_img_del_storage->delete_object1('av8db','th2_'.$old_logo['company_logo'],'',$src2);
                      }
                      $main_img_three = $company_img_del_storage->getImageUrl1('av8db','th3_'.$old_logo['company_logo'],$src2);
                      $is_main_img_three = getimagesize($main_img_three);
                      if(!empty($is_main_img_three)){
                        $del2 = $company_img_del_storage->delete_object1('av8db','th3_'.$old_logo['company_logo'],'',$src2);
                      }
                    }
                    // if(file_exists(DIR_UPD_COMPANY_LOGOS.$old_logo['company_logo'])){
                    //     unlink(DIR_UPD_COMPANY_LOGOS.$old_logo['company_logo']);
        ...
        if (isset($_FILES['banner_image']) && !($_FILES['banner_image']['error']) && $platform == 'app') {
            ...
            if ($_POST['is_banner_removed'] == "true") {
                ...
                if($old_logo['banner_image'] != ''){
                    //delete image from bucket if old image is exists in bucket
                    $src2 = 'company-banner-images-nct/';
                    if ($old_logo['banner_image'] != '') {
                      $main_img = $company_img_del_storage->getImageUrl1('av8db',$old_logo['banner_image'],$src2);
                      $is_main_img = getimagesize($main_img);
                      if(!empty($is_main_img)){
                        $del = $company_img_del_storage->delete_object1('av8db',$old_logo['banner_image'],'',$src2);
                      }
                      $main_img_one = $company_img_del_storage->getImageUrl1('av8db','th1_'.$old_logo['banner_image'],$src2);
                      $is_main_img_one = getimagesize($main_img_one);
                      if(!empty($is_main_img_one)){
                        $del1 = $company_img_del_storage->delete_object1('av8db','th1_'.$old_logo['banner_image'],'',$src2);
                      }
                    }
                    // if(file_exists(DIR_UPD_COMPANY_BANNER_IMAGES.$old_logo['banner_image'])){
                    //     unlink(DIR_UPD_COMPANY_BANNER_IMAGES.$old_logo['banner_image']);
