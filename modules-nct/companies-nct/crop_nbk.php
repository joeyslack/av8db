    ...
    if ($errorCode === UPLOAD_ERR_OK) {
      ...
      //  delete_directory($path);
        ...
        if ($type == IMAGETYPE_GIF || $type == IMAGETYPE_JPEG || $type == IMAGETYPE_PNG) {
              ...
              $create_com_storage = new storage();
              echo "in";
              if (!extension_loaded('imagick')) {
                ...
                echo "sd";
                  ...
                  $main_img = $create_com_storage->upload_object('av8db','',$tempname,$temp_src);
                  echo "<pre>";print_r('here 12')
                  $get_main_img = $create_com_storage->getImageUrl1('av8db',$img_name,$temp_src2);
                  echo "<pre>url ";print_r($get_main_img);
              } 
              if ($get_main_img) {
...
echo "<pre>out main url";print_r($main_url);
$pro_pic_name=$crop -> filename();

$crop_img_res = '';
$create_company_logo = new storage();
...
$crop_img = $create_company_logo->upload_objectBlob('av8db',$pro_pic_name,$im->getImageBlob(),$company_logo);
$crop_img_res = $create_company_logo->getImageUrl1('av8db',$pro_pic_name,$company_logo);
echo "<pre> crop img url";print_r($crop_img_res);
if ($crop->new_data->rotate > 0) {
    $im->rotateimage('', $crop->new_data->rotate);
    $crop_img = $create_company_logo->upload_objectBlob('av8db',$pro_pic_name,$im->getImageBlob(),$company_logo);
    $crop_img_res = $create_company_logo->getImageUrl1('av8db',$pro_pic_name,$company_logo);
}
$final_url = $crop_img_res;
...
echo "<pre> final res";print_r($response);
...
echo "<pre>sess ver";print_r($_SESSION);
