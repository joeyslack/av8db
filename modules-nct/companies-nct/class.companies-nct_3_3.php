...
class Companies extends Home {
    ...
    public function getCompanies($user_id, $type, $currentPage = 1, $getPagination = true,$platform = 'web') {
        ...
        if ('my_companies' == $type) {
            ...
            $query = " FROM tbl_companies comp LEFT JOIN tbl_industries i ON i.id = comp.company_industry_id WHERE comp.user_id = ? AND comp.status = ? AND comp.company_type = ? AND comp.isAdminVerify = ? ORDER BY comp.id DESC ";
            //$query = " FROM tbl_companies comp LEFT JOIN tbl_company_sizes cs ON cs.id = comp.company_size_id LEFT JOIN tbl_industries i ON i.id = comp.company_industry_id WHERE comp.user_id = ? AND comp.status = ? AND comp.company_type = ? ORDER BY comp.id DESC ";

            $where_arr=array($user_id,'a','r','y');
        } else {
            $data_selection_query = "SELECT comp.company_description,comp.id,comp.company_logo,comp.company_name,comp.website_of_company,comp.owner_email_address, i.industry_name_".$this->lId." as industry_name ";
            $count_selection_query = "SELECT count(comp.id) as no_of_companies ";
            $query = " FROM tbl_company_followers cf LEFT JOIN tbl_companies comp ON comp.id = cf.company_id LEFT JOIN tbl_industries i ON i.id = comp.company_industry_id WHERE cf.user_id = ? AND comp.status = ? AND comp.company_type = ? AND comp.isAdminVerify = ? ORDER BY comp.id DESC ";
            //$query = " FROM tbl_company_followers cf LEFT JOIN tbl_companies comp ON comp.id = cf.company_id LEFT JOIN tbl_company_sizes cs ON cs.id = comp.company_size_id LEFT JOIN tbl_industries i ON i.id = comp.company_industry_id WHERE cf.user_id = ? AND comp.status = ? AND comp.company_type = ? ORDER BY comp.id DESC ";
            $where_arr=array($user_id,'a','r','y');
        }
        ...
        if ($getShowableResults) {
           ...
           // echo "<pre>";print_r($getShowableResults);exit;
            for ($i = 0; $i < count($getShowableResults); $i++) {
                ...
                $company_logo_url=getImageURL("company_logo", filtering($getShowableResults[$i]['id'], 'output', 'int'), "th2");
                if($platform == 'web'){
