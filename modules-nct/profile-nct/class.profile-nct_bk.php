...
class Profile extends Home {
    function __construct($platform='web',$current_user_id=0) {
        ...
        $query = "SELECT u.profile_picture_name,u.id,u.first_name,u.last_name,u.email_address,u.date_added,u.phone_no,u.user_home_airport,u.isFerryPilot,u.personal_details,u.user_DOB,l.formatted_address, i.industry_name_".$this->lId." as industry_name,i.id as industry_id,
    ...
    public function addExperience($platform='web') {
     ...
     public function addLicenses($platform='web') {
       //echo "<pre>";print_r($_POST);exit();
        ...
        //print_r($licenses_id);exit();
        $licenses_ids = decryptIt($_POST['licenses_id']);
        $licenses_name = filtering($_POST['licenses_name']);
        $date_obtain = $_POST['date_obtain'];
        $institute_name = filtering($_POST['institute_name'], 'input', 'int');
       ...
       if ($licenses_id > 0) {
        ...
        if ($licenses_id1 > 0) {
          //  $licenses_details_array['licenses_id'] = $licenses_id1;
            if ($licenses_id > 0) {
                $affectedRows = $this->db->update("tbl_users_licenses_endorsement", $licenses_details_array, array("id" => $experience_id))->affectedRows();
                if ($affectedRows > 0 || $affectedRows == 0) {
                    $response['status'] = true;
                    $response['success'] = SUCCESS_YOUR_EXPERIENCE_DETAILS_HAS_BEEN_UPDATED_SUCCESSFULLY;
                    $response['licenses'] = $this->getAddedLicensesEndorsement($post_user_id);
                ...
                $licenses_details_array['user_id'] = $post_user_id;
                $licenses_details_array['licenses_id'] = $licenses_id;
                $licenses_details_array['date_obtained'] = $date_obtain;
                $licenses_details_array['institute_name'] = $institute_name;
                $licenses_details_array['verification_status'] = 'n';
                $licenses_details_array['country_id'] = $country_id;
                $licenses_details_array['added_on'] = date("Y-m-d H:i:s");
                $added_id =$this->db->insert("tbl_users_licenses_endorsement", $licenses_details_array)->getLastInsertId();
                $response['status'] = true;
                $response['success'] = SUCCESS_YOUR_EXPERIENCE_DETAILS_HAS_BEEN_ADDED_SUCCESSFULLY;
                $response['licenses'] = $this->getAddedLicensesEndorsement($post_user_id);
                $response['current_experience_app'] = $this->getAddedLicensesEndorsement($post_user_id,'app',$added_id);
                return $response;
            }
        } else {
            $response['error'] = ERROR_OOPS_SOMETHING_WENT_WRONG_TRY_AFTER_SOME_TIME;
    ...
    public function getAddedLicensesEndorsement($user_id,$platform = 'web') {
        $final_result = '';
        $query = "SELECT ue.*,l.licenses_endorsements_name_".$this->lId." as license_name FROM tbl_users_licenses_endorsement ue LEFT JOIN tbl_license_endorsements l ON ue.licenses_id = l.id WHERE ue.user_id = ? ";
        ...
        if ($licenses != '') {
            ...
            $single_education_tpl->set('delete_actions', $delete_actions);
            $single_education_tpl_parsed = $single_education_tpl->parse();
            $fields = array("%LICENSES_ID_ENCRYPTED%","%LICENSE_NAME%","%DATE_OBTAIN%","%INSTITUTE_NAME%","%VERIFICATION_STATUS%");
            for ($i = 0; $i < count($licenses); $i++) {
                ...
                $verification_status = filtering($licenses[$i]['verification_status']);
                ...
                if($platform == 'app'){
                   // $array[] = array('education_id'=>$licenses[$i]['id'],'university_name'=>$university_name,'degree_name'=>$degree_name,'field_of_study'=>$field_of_study,'grade_or_percentage'=>$grade_or_percentage,'from_year'=>$from_year,'to_year'=>$to_year,'description'=>$description);
                } else {
                    $final_result .= str_replace($fields, $fields_replace, $single_education_tpl_parsed);
                }
            }
    ...
    public function getAddedLicensesEndorsementForm($licenses_id = '') {
        ...
        $industry_options = $this->getCountryOptions();
        if($licenses_id > 0){
            $query = "SELECT ue.*,l.licenses_endorsements_name_".$this->lId." as license_name FROM tbl_users_licenses_endorsement ue LEFT JOIN tbl_license_endorsements l ON ue.licenses_id = l.id";
        ...
        $fields_replace = array(
            ...
            $industry_options,
            $experience_details['verification_status'],           
        );
    ...
    public function getUserDetailForm($education_id = '') {
        ...
        $fields = array("%FIRST_NAME%","%LAST_NAME%","%FORMATTED_ADDRESS%","%USER_LOCATION%","%ADDRESS1%","%ADDRESS2%","%COUNTRY%","%STATE%","%CITY1%","%CITY2%","%POSTAL_CODE%","%LATITUDE%","%LONGITUDE%","%CREATED_DATE%","%USER_EMAIL_ADDRESS%","%USER_CONTACT_NO%");
        $fields_replace = array($this->first_name,$this->last_name,$this->formatted_address,$this->formatted_address,$this->address1,$this->address2,$this->country,$this->state,$this->city1,$this->city2,$this->postal_code,$this->latitude,$this->longitude,$this->createdAt,$this->email_address,$this->phone_no);
        //echo "<pre>";print_r($fields_replace);exit();
    ...
    public function getLicensesForSuggestion($user_id, $licenses_name='',$platform='web') {
        ...
        $query = "SELECT id,licenses_endorsements_name_".$this->lId." as licenses_endorsements_name FROM tbl_license_endorsements WHERE isActive = ? ORDER BY id DESC ";
        $where_arr=array('y');
    ...
    public function getPageContent($platform="web") {
        ...
        $url_user_id = decryptIt($_GET['user_id']);
        ...
        $language_hide=$experiences_hide=$educations_hide=$skill_hide=$licenses_hide='';
        ...
        $no_data_hide='hidden';
        if($language=='' && $experiences=='' && $educations=='' && $licenses == '' && $skill=='' && $this->user_id != $_SESSION['user_id']){
            $no_data_hide='';
        }
    ...
    public function storeFerryPilotRateReview($rating,$desc){
        ...
        $send_arr['receiver_id'] = $this->session_user_id;
