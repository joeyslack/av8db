   ...
   // $_SESSION['temp_files']=$pro_pic_name;
    if($pro_pic_name !="" || $pro_pic_name !=NULL){
      ...
      if ($this->user_id > 0) {
        ...
        $old_profile_picture_name=getTableValue("tbl_users", "profile_picture_name", array("id" => $this->user_id));

        $profile_picture_name = $pro_pic_name;

        $affected_rows = $db->update("tbl_users", array("profile_picture_name" => $profile_picture_name,"date_updated" => date("Y-m-d H:i:s")), array("id" => $this->user_id))->affectedRows();
           
        $result1 = $storage->getImageUrl1('av8db','th4_'.$pro_pic_name,$src2);
        if ($affected_rows) {
            $del = $storage->delete_object('av8db',$pro_pic_name,'');
            $first_name = getTableValue('tbl_users','first_name',array('id'=>$this->user_id));
            $last_name = getTableValue('tbl_users','last_name',array('id'=>$this->user_id));
  ...
  // $_SESSION['user_id'] = $this->user_id;
    if($pro_pic_name !="" || $pro_pic_name !=NULL){
        ...
        if ($this->user_id > 0) {
            ...
            $old_profile_picture_name=getTableValue("tbl_users", "cover_photo", array("id" => $this->user_id));
           ...
           // echo "<pre>";print_r($old_profile_picture_name);
            ...
            $affected_rows = $db->update("tbl_users", array("cover_photo" => $profile_picture_name,"date_updated" => date("Y-m-d H:i:s")), array("id" => $this->user_id))->affectedRows();
