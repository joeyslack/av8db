  ...
  private function setFile($file) {
    ...
    if ($errorCode === UPLOAD_ERR_OK) {
      ...
      //  delete_directory($path);
        ...
        //$src = DIR_UPD."temp_files/".$img_name;
        ...
        if ($type == IMAGETYPE_GIF || $type == IMAGETYPE_JPEG || $type == IMAGETYPE_PNG) {
              ...
              if (isset($file['tmp_name'])) {
                ...
                $result = $storage->upload_object('av8db','',$tempname,$src);
                $res12 = $storage->getImageUrl1('av8db',$img_name,$src2);
                echo "<pre>";print_r($res12);
                //$res12 = $storage->getImageUrl('av8db',$img_name);

                $path_to_your_files = $res12;
                echo "<pre>";print_r($path_to_your_files);

                $image_file_name = $filename;
                $targetdir = $path_to_your_files;
                $my_image = $path_to_your_files;
                echo "<pre>";print_r($my_image);
                $im = new Imagick($my_image);
                echo "Created object...";
                $im->readImage($my_image);
                // $im->readImageBlob($my_image);
                echo "Read JPG...";
                
                $im->cropImage($this->data->width, $this->data->height, $this->data->x, $this->data->y); 
                echo "<pre>";echo "cropped images";echo "<pre>";
                //$im->writeImage($targetdir.'cropped_'.$filename); 

                $result = $storage->upload_objectBlob('av8db','cropped_'.$img_name,$im->getImageBlob(),$src2);
                echo "<pre>";print_r('stored cropped');
                $im->resizeImage( 250, 80, Imagick::FILTER_LANCZOS, 1);
                // $im->writeImage($targetdir.'th1_'.$filename);
                $resultr = $storage->upload_objectBlob('av8db','resize_'.$img_name,$im->getImageBlob(),$src2);
                print_r($im);
                $im->clear();
                $im->destroy(); 
                echo "Done.";
