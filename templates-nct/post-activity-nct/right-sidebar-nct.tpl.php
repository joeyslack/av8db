<?php 
    $users_profile_picture1 = $upro = '';
    require_once(DIR_MOD . 'common_storage.php');
    $user_c_post = new storage();
   
    $src2 = DIR_NAME_USERS."/".$_SESSION['user_id']."/";

    $cover_nm1 = getTableValue("tbl_users", "profile_picture_name", array("id" => $_SESSION['user_id']));
    $users_profile_picture1 = $user_c_post->getImageUrl1('av8db','th4_'.$cover_nm1,$src2);
    $up = getimagesize($users_profile_picture1);
    if (!empty($up)) {
        $upro = '<picture>
                    <source srcset="' . $users_profile_picture1 . '" type="image/jpg">
                    <img src="' . $users_profile_picture1 . '" class="" alt="' . $_SESSION['first_name'] . '" /> 
                </picture>';
    }else{
        $upro = '<span class="profile-picture-character">' . ucfirst(mb_substr($_SESSION['first_name'], 0, 1, 'utf-8')) . '</span>'; 
    }
?>
...
<div class="fix-sidebar" data-spy="affix" data-offset-top="0" data-offset-bottom="30">
  ...
  <div class="profile-view-outer">
      ...
      <figure>
        <div class="profile-pic"><?php echo $upro; ?>
        </div>
