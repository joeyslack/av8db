...
function getGroupCarouselItem($group_id, $active_class, $group_storage = '') {
    global $db;
    $final_content = '';
    ...
    if ($group_details) {
        foreach ($group_details as $key => $value) {
            ...
            $group_logo_image_name = getTableValue('tbl_groups','group_logo',array('id'=>$value['id']));
            $group_logo = $group_storage->getImageUrl1('av8db','th2_'.$group_logo_image_name,'group-logos-nct/');
            $is_image = getimagesize($group_logo);
            if(!empty($is_image)){
                $group_logo = '<img src="'.$group_logo.'" alt="'.$value['group_name'].'">';
            }else{
                $group_logo = '<span class="profile-picture-character">'.ucfirst(mb_substr($value['group_name'], 0, 1, 'utf-8')).'</span>';
            }
...
function getCompanyCarouselItem($company_id, $active_class, $company_storage = '') {
    global $db,$lId;
    $final_content = '';
    ...
    if ($company_details) {
        foreach ($company_details as $key => $value) {
            ...
            $company_logo_image_name = getTableValue('tbl_companies','company_logo',array('id'=>$value['id']));
            $company_logo = $company_storage->getImageUrl1('av8db','th2_'.$company_logo_image_name,'company-logos-nct/');
            $is_image = getimagesize($company_logo);
            if(!empty($is_image)){
                $company_logo = '<img src="'.$company_logo.'" alt="'.$value['company_name'].'">';
            }else{
                $company_logo = '<span class="profile-picture-character">'.ucfirst(mb_substr($value['company_name'], 0, 1, 'utf-8')).'</span>';
            }
...
function getJobCarouselItem($job_id, $active_class, $job_storage = '') {
    global $db,$lId;
    $final_content = '';
    ...
    if ($job_details) {
        foreach ($job_details as $key => $value) {
            ...
            $job_logo_image_name = getTableValue('tbl_companies','company_logo',array('id'=>$value['company_id']));
            $image = $job_storage->getImageUrl1('av8db','th2_'.$job_logo_image_name,'company-logos-nct/');
            $is_image = getimagesize($image);
            if(!empty($is_image)){
                $image = '<img src="'.$image.'" alt="'.$value['company_name'].'">';
            }else{
                $image = '<span class="profile-picture-character">'.ucfirst(mb_substr($value['company_name'], 0, 1, 'utf-8')).'</span>';
            }
            $fields_replace = array(
                $active_class,
                encryptIt(filtering($value['id'], 'output', 'int')),
...
function handlePaypalPaymentResponse($paypal_response_array) {
    ...
    // if ($paypal->validate_ipn()) {
        // || (isset($paypal_response_array['platform']) && $paypal_response_array['platform'] == 'app')
...
function getSingleFeed($feed_storage = '',$feed_id,$platform='web',$current_user_id,$module='',$feed_act_id=0) {
    ...
    if ($feed_details) {
       ...
       // $fields_company_logo = array("%IMG%");
        ...
        if ($feed_details['image_name'] != "") {
            ...
            $image_url = $feed_storage->getImageUrl1('av8db',$image_name,'feed-images-nct/');
            $post_image_tpl = new Templater(DIR_TMPL . "post-image-nct.tpl.php");
            $post_image_tpl_parsed = $post_image_tpl->parse();
            $fields_post_image = array("%IMAGE_NAME%", "%IMAGE_URL%");
            $fields_replace_post_image = array($image_name, $image_url);
        ...
        if ($feed_details['company_id']) {
            ...
            $company_logo_image_name = getTableValue('tbl_companies','company_logo',array('id'=>$feed_details['company_id']));
            $app_img_url = $company_logo_url = $feed_storage->getImageUrl1('av8db','th2_'.$company_logo_image_name,'company-logos-nct/');
            $is_image = getimagesize($company_logo_url);
            $company_name = getTableValue('tbl_companies','company_name',array('id'=>$feed_details['company_id']));
            if(!empty($is_image)){
                $company_logo_url = '<img src="'.$company_logo_url.'" alt="'.$company_name.'">';
            }else{
                $company_logo_url = '<span class="profile-picture-character">'.ucfirst(mb_substr($company_name, 0, 1, 'utf-8')).'</span>';
            }
            ...
            $app_img_url = $profile_picture = $feed_storage->getImageUrl1('av8db','th2_'.$profile_picture_name,'users-nct/'.$feed_details['user_id'].'/');
            $is_image = getimagesize($profile_picture);
            if(!empty($is_image)){
                $profile_picture = '<img src="'.$profile_picture.'" alt="'.$first_name.' '.$last_name.'">';
            }else{
                $profile_picture = '<span class="profile-picture-character">'.ucfirst(mb_substr($first_name, 0, 1, 'utf-8')).'</span>';
            }
        ...
        $fields_replace = array(
            ...
            $feedDescription,
            $post_image,
            getCommentsContainer($feed_storage,$feed_id),
            $timestamp,
...
function getSingleCommentBox($comment_storage = '',$feed_id, $comment_id,$platform='web') {
    global $db;
    ...
    if ($comment) {
        ...
        $user_pro_pic_name = getTableValue('tbl_users','profile_picture_name',array('id'=>$comment['user_id']));
        $uImage = $comment_storage->getImageUrl1('av8db','th2_'.$user_pro_pic_name,'users-nct/'.$comment['user_id'].'/');
        $is_image = getimagesize($uImage);
        if(!empty($is_image)){
            $uImage = '<img src="'.$uImage.'" alt="'.$first_name.' '.$last_name.'">';
        }else{
            $uImage = '<span class="profile-picture-character">'.ucfirst(mb_substr($first_name, 0, 1, 'utf-8')).'</span>';
        }
...
function getComments($comment_storage = '',$feed_id, $currentPage = 1,$platform='web') {
    global $db;
    $response = array();
    $response['status'] = false;
    $comments_html = "";
    ...
    if ($comments) {
        ...
        for ($i = 0; $i < count($comments); $i++) {
            ...
            $getSingleCommentBox = getSingleCommentBox($comment_storage,$feed_id, $comment_id,$platform);
            if($platform == 'app'){
...
function getCommentsContainer($comment_storage = '',$feed_id) {
    global $db;
    $final_result = "";
    ...
    $response = getComments($comment_storage,$feed_id);
