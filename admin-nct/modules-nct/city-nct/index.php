    ...
    if ($objPost->cityName != "") {
        if ($type == 'edit' && $id > 0) {
            if (in_array('edit', $Permission)) {
                $db->update($table, array('cityName' => $objPost->cityName, 'StateID' => $objPost->StateID, 'CountryID' => $objPost->CountryID, 'isActive' => $objPost->isActive), array("CityId" => $id));
            ...
            if (in_array('add', $Permission)) {
                ...
                if (getTotalRows("tbl_city", "CountryID='".$objPost->CountryID."' AND StateID = '".$objPost->StateID."' AND cityName='" . $objPost->cityName . "'", 'CityId') == 0) {
