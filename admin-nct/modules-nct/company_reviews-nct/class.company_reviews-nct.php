...
class CompanyReviews extends Home {
    ...
    public function __construct($module, $id = 0, $objPost = NULL, $searchArray = array(), $type = '') {
        ...
        $this->fields = $fields;
        $this->module = $module;
        $this->table = 'tbl_company_rate_review';
        ...
        if ($this->id > 0) {
           ...
           // _print_r($this->data);exit();
            $this->data['review_description'] = $this->review_description = filtering($fetchRes['review_description']);
    ...
    public function dataGrid() {
        ...
        $whereCond = '';
        if (isset($chr) && $chr != '') {
            $whereCond .= " AND (review_description LIKE '%" . $chr . "%' OR rating LIKE '%".$chr."%' OR company_name LIKE '%".$chr."%')";
        }
        if (isset($sort))
            $sorting = $sort . ' ' . $order;
        else
       ...
       $sql = "SELECT c.*,cp.company_name,u.first_name,u.last_name FROM tbl_company_rate_review as c LEFT JOIN tbl_companies as cp ON c.company_id = cp.id LEFT JOIN tbl_users as u ON c.sender_id=u.id WHERE c.isReported = 'y' " . $whereCond . " order by " . $sorting;
        ...
        foreach ($qrySel as $fetchRes) {
            ...
            $rating = isset($fetchRes['rating']) ? $fetchRes['rating'] : '-';
