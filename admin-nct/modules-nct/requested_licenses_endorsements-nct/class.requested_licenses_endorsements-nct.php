...
class RequestedLicensesEndorsements extends Home {
    ...
    public function dataGrid() {
        ...
        $whereCond = '';
        if (isset($chr) && $chr != '') {
            $whereCond .= " AND (licenses_endorsements_name_".DEFAULT_LANGUAGE_ID." LIKE '%" . $chr . "%' OR u.first_name LIKE '%" . $chr . "%' OR u.last_name LIKE '%" . $chr . "%')";
        }
        if (isset($sort))
            $sorting = $sort . ' ' . $order;
        else
        ...
        $sql = "SELECT l.*,u.first_name,u.last_name FROM tbl_users_licenses_endorsement as ul LEFT JOIN tbl_license_endorsements as l ON ul.licenses_id = l.id LEFT JOIN tbl_users as u ON u.id=ul.user_id WHERE ul.user_id > 0 " . $whereCond . " order by " . $sorting;
        ...
        foreach ($qrySel as $fetchRes) {
            ...
            if (in_array('status', $this->Permission)) {
                ...
                $final_array = array_merge($final_array);
            }
