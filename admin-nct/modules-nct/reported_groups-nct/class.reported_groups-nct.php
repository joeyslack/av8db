...
class ReportedGroups extends Home {
    ...
    public function __construct($module, $id = 0, $objPost = NULL, $searchArray = array(), $type = '') {
        ...
        if ($this->id > 0) {
            ...
            $fetchRes = $qrySel;
            $this->data=$fetchRes;
            $this->data['group_name'] = $this->group_name = filtering($fetchRes['group_name']);   
        } else {
    ...
    public function dataGrid() {
        ...
        $whereCond = '';
        if (isset($chr) && $chr != '') {
            $whereCond .= " AND (group_name LIKE '%" . $chr . "%' OR first_name LIKE '%".$chr."%' OR last_name LIKE '%".$chr."%')";
        }
        if (isset($sort))
            $sorting = $sort . ' ' . $order;
        else
       ...
       $sql = "SELECT g.*,u.first_name,u.last_name FROM tbl_groups as g LEFT JOIN tbl_users as u ON g.user_id = u.id WHERE g.isGroupReported = 'y' " . $whereCond . " order by " . $sorting;
        ...
        foreach ($qrySel as $fetchRes) {
            ...
            $group_name       = isset($fetchRes['group_name']) ? $fetchRes['group_name'] : '-';
            $user_name           = $fetchRes['first_name']. ' ' .$fetchRes['last_name'];
            
            $final_array = array(
                filtering($id),
                $user_name,
                filtering($group_name)
            );
