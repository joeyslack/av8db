...
class Industries extends Home {
    ...
    public function __construct($module, $id = 0, $objPost = NULL, $searchArray = array(), $type = '') {
        ...
        if ($this->id > 0) {
            ...
            $this->data['industry_description'] = $this->industry_description = filtering($fetchRes['industry_description']);
            ...
            $this->data['industry_name'] = $this->industry_name = '';
            $this->data['industry_description'] = $this->industry_description = '';
    ...
    public function viewForm() {
        ...
        foreach ($lang as $key => $value) {
            $name_content .= $this->displayBox(array("label" => "Company Type(".$value['languageName'].") &nbsp;:", "value" => $this->data['industry_name_'.$value['id']]));
            $description_content .= $this->displayBox(array("label" => "Description (".$value['languageName'].") &nbsp;:", "value" => $this->data['industry_description_'.$value['id']]));
        }
    ...
    public function getForm() {
        ...
        foreach ($this->get_languages as $lang) {
            $box_label = MEND_SIGN.'Company Type Title('.$lang['languageName'].')';
            $box_field_name = 'industry_name';
            $box_field_value = $this->id > 0 ? $this->data['industry_name_'.$lang['id']] : '';
            $replace = array($lang['id'],$box_label,$box_field_name,$box_field_value);
            $industry_name .= str_replace($search, $replace, $textbox);

            $area_label = MEND_SIGN.'Description('.$lang['languageName'].')';
            $area_field_name = 'industry_description';
            $area_field_value = $this->id > 0 ? $this->data['industry_description_'.$lang['id']] : '';
            $area_replace = array($lang['id'],$area_label,$area_field_name,$area_field_value);
            $industry_description .= str_replace($search, $area_replace, $textarea);
        }
        $fields = array("%MEND_SIGN%","%STATUS_A%","%STATUS_D%","%TYPE%","%ID%",'%INDUSTRY_NAME%','%INDUSTRY_DESCRIPTION%');
        $fields_replace = array(MEND_SIGN,$status_a,$status_d,$this->type,$this->id,$industry_name,$industry_description);
    ...
    public function dataGrid() {
        ...
        $whereCond = '';
        if (isset($chr) && $chr != '') {
            $whereCond .= " WHERE ( industry_name LIKE '%" . $chr . "%' OR industry_description LIKE '%" . $chr . "%' )";
        }
        ...
        foreach ($qrySel as $fetchRes) {
            ...
            $is_default = (isset($fetchRes['isDefault']))?$fetchRes['isDefault']:"";
            ...
            $final_array = array(
                ...
                filtering($fetchRes["industry_name_".DEFAULT_LANGUAGE_ID])
            );
