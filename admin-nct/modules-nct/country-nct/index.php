...
if (isset($_POST["submitAddForm"]) && $_SERVER["REQUEST_METHOD"] == "POST") {
    extract($_POST);
    $objPost->countryName = isset($countryName) ? filtering($countryName, 'input') : '';
    $objPost->isActive = isset($isActive) && $isActive == 'y' ? 'y' : 'n';

    if ($objPost->countryName != "") {
        if ($type == 'edit' && $id > 0) {
            if (in_array('edit', $Permission)) {
                $db->update($table, array('countryName' => $objPost->countryName, 'isActive' => $objPost->isActive), array("CountryId" => $id));
            ...
            if (in_array('add', $Permission)) {
                if (getTotalRows($table, "countryName='" . $objPost->countryName . "'", 'CountryId') == 0) {
...
$objCountry = new Country($module, $id, NULL);
