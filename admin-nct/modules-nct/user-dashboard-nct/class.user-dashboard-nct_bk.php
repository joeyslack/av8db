...
class User_dashboard extends Home {
            ...
            foreach($getShowableResults as $single_company) {
                if ($single_company['company_logo'] == '') {
                   ...
                   /* $company_logo=SITE_THEME_IMG . "no-image.jpg";
                    ...
                    $company_logo = $company_storage_logo->getImageUrl1('av8db','th2_'.$single_company['company_logo'],$src2);
        ...
        if($getShowableResults) {
            ...
            foreach($getShowableResults as $single_company) {
                if ($single_company['company_logo'] == '') {
                    ...
                    $company_logo = $following_storage_logo->getImageUrl1('av8db','th2_'.$single_company['company_logo'],$src2);
        ...
        if($getShowableResults) {
            ...
            foreach($getShowableResults as $single_job) {
                ...
                if (!empty($get_company_details)) {
                    ...
                    $up = getimagesize($src2.'th2_'.$company_logo_name);
                    if ($company_logo_name == '' || empty($up)) {
                        $company_logo = "https://storage.googleapis.com/av8db/no-image.jpg";
                    }else 
                    {   
        ...
        if($getShowableResults) {
            ...
            foreach($getShowableResults as $single_group) {
                ...
                if (!empty($get_group_details)) {
                    ...
                    $up = getimagesize($src2.'th2_'.$group_logo_name);
                    if ($group_logo_name == '' || empty($up)) {
    ...
    public function getJoinedGroups($user_id, $currentPage = 1) {
        ...
        if($getShowableResults) {
            ...
            foreach($getShowableResults as $single_group) {
                ...
                $user_src = DIR_NAME_USERS."/".$this->id."/";
                ...
                if (!empty($get_group_user_details)) {
                    $group_user_name = filtering($get_group_user_details['profile_picture_name']);

                    $up1 = getimagesize($user_src.'th2_'.$group_user_name);
                    if ($group_user_name == '' || empty($up1)) {
                    ...
                    {   
                        $img_url = $mygroup_storage_logo->getImageUrl1('av8db','th2_'.$group_user_name,$user_src);
        ...
        if($connections_array) {
            ...
            foreach ($connections_array as $key => $value) {
                ...
                if (!empty($get_group_user_details)) {
                    ...
                    {   
                        $img_url = $connection_user->getImageUrl1('av8db','th2_'.$group_user_name,$user_src);
                        $group_user_logo ='<picture>
                                            <source srcset="' . $img_url . '" type="image/jpg">
                                            <img src="' . $img_url . '" class="" alt="img" /> 
                                        </picture>';
    ...
    public function getPageContent() {
        ...
        echo "<pre>";print_r($users_profile_picture);
        $up1 = getimagesize($src2.'th4_'.$users_profile_picture);
         if (empty($up1)) {
        ...
        echo "<pre>";print_r($users_profile_picture_url);
        $user_cover_img = $user_dashboard_view->getImageUrl1('av8db','th1_'.$this->cover_photo,$src2);
        $up = getimagesize($src2.'th1_'.$user_cover_img);
        if (empty($up)) {
