...
class Group_type extends Home {
    ...
    public function __construct($module, $id = 0, $objPost = NULL, $searchArray = array(), $type = '') {
        ...
        if ($this->id > 0) {
            ...
            $this->data['group_type'] = $this->group_type = filtering($fetchRes['group_type']);
            $this->data['group_type_description'] = $this->group_type_description = filtering($fetchRes['group_type_description']);
            $this->data = $fetchRes;
            ...
            $this->data['group_type'] = $this->group_type = '';
            $this->data['group_type_description'] = $this->group_type_description = '';
    ...
    public function viewForm() {
        ...
        foreach ($lang as $key => $value) {
            $name_content .= $this->displayBox(array("label" => "Group Type(".$value['languageName'].") &nbsp;:", "value" => $this->data['group_type_'.$value['id']]));
            $description_content .= $this->displayBox(array("label" => "Description (".$value['languageName'].") &nbsp;:", "value" => $this->data['group_type_description_'.$value['id']]));
        }
    ...
    public function getForm() {
        ...
        //_print_r($this->get_languages);
        $search = array('%ID%','%LABEL%','%FIELD_NAME%','%FIELD_VALUE%');


        foreach ($this->get_languages as $lang) {
            $box_label = MEND_SIGN.'Group Type('.$lang['languageName'].')';
            $box_field_name = 'group_type';
            $box_field_value = $this->id > 0 ? $this->data['group_type_'.$lang['id']] : '';
            $replace = array($lang['id'],$box_label,$box_field_name,$box_field_value);
            $group_type .= str_replace($search, $replace, $textbox);

            $area_label = MEND_SIGN.'Group Description('.$lang['languageName'].')';
            $area_field_name = 'group_description';
            $area_field_value = $this->id > 0 ? $this->data['group_type_description_'.$lang['id']] : '';
            $area_replace = array($lang['id'],$area_label,$area_field_name,$area_field_value);
            $group_description .= str_replace($search, $area_replace, $textarea);
        }
        $fields = array("%MEND_SIGN%","%STATUS_A%","%STATUS_D%","%TYPE%","%ID%",'%GROUP_TYPE%','%GROUP_DESCRIPTION%','%requried%');
        $fields_replace = array(MEND_SIGN,$status_a,$status_d,$this->type,$this->id,$group_type,$group_description,'required');
    ...
    public function dataGrid() {
        ...
        $whereCond = '';
        if (isset($chr) && $chr != '') {
            $whereCond .= " WHERE ( group_type_".DEFAULT_LANGUAGE_ID." LIKE '%" . $chr . "%' OR group_type_description LIKE '%" . $chr . "%' )";
        }
        ...
        foreach ($qrySel as $fetchRes) {
            ...
            $final_array = array(
                ...
                filtering($fetchRes["group_type_".DEFAULT_LANGUAGE_ID])
            );
