...
class RequestedLicensesEndorsements extends Home {
    ...
    public function dataGrid() {
        ...
        $whereCond = '';
        if (isset($chr) && $chr != '') {
            $whereCond .= " AND (formatted_address LIKE '%" . $chr . "%' OR u.first_name LIKE '%" . $chr . "%' OR u.last_name LIKE '%" . $chr . "%')";
        }
        if (isset($sort))
            $sorting = $sort . ' ' . $order;
        else
        ...
        $sql = "SELECT l.*,u.first_name,u.last_name FROM tbl_users as u
        LEFT JOIN tbl_locations as l ON u.location_id = l.id WHERE u.location_id > 0  AND u.id > 0" . $whereCond . " order by " . $sorting;
        ...
        foreach ($qrySel as $fetchRes) {
            ...
            $location=$fetchRes['city1'].', '.$fetchRes['state'].', '.$fetchRes['country'];
            $final_array = array(
                filtering($id),
                filtering($fetchRes["formatted_address"]),
                $location,
                $user_name
            );
