...
class State extends Home {
    ...
    public function __construct($module, $id = 0, $objPost = NULL, $searchArray = array(), $type = '') {
        ...
        parent::__construct();
        if ($this->id > 0) {
            $qrySel = $this->db->select($this->table, array("StateID", "CountryID", "stateName", "isActive"), array("StateID" => $id))->result();
            $fetchRes = $qrySel;
            $this->data['StateID'] = $this->StateID = filtering($fetchRes['StateID'], 'output', 'int');
            $this->data['CountryID'] = $this->CountryID = filtering($fetchRes['CountryID'], 'output', 'int');
    ...
    public function viewForm() {
        ...
        $content = $this->displayBox(array("label" => "State name &nbsp;:", "value" => $this->stateName)).
        $this->displayBox(array("label" => "Status&nbsp;:", "value" => $this->isActive == 'y' ? 'Active' : 'Deactive'));
        return $content;
    }

    public function getForm() {
        $content = $country_option = '';
        ...
        foreach ($qrySelCountry as $fetchRes) {
            ...
            $fields_replace = array($fetchRes['CountryId'], $selected, filtering($fetchRes['countryName']));
        ...
        $fields = array("%MEND_SIGN%", "%COUNTRY_OPTION%", "%STATE_NAME%", "%STATUS_A%", "%STATUS_D%", "%TYPE%", "%ID%");

        $fields_replace = array(MEND_SIGN, $country_option, $this->data['stateName'], $status_a, $status_d, $this->type, $this->id);
    ...
    public function dataGrid() {
        ...
        if (isset($sort))
            $sorting = (in_array($sort, array('countryName')) ? 'c.' : 's.') . $sort . ' ' . $order;
        else
            $sorting = 's.StateID DESC';

        $totalRowTmp = $this->db->pdoQuery("SELECT COUNT(s.StateID) AS nmrows FROM tbl_state AS s INNER JOIN tbl_country AS c ON s.CountryID = c.CountryId $sWhere", $aWhere)->result();
        $totalRow = $totalRowTmp['nmrows'];

        $qrySel = $this->db->pdoQuery("SELECT s.*,c.countryName FROM tbl_state AS s INNER JOIN tbl_country AS c ON s.CountryID = c.CountryId $sWhere ORDER BY $sorting limit $offset , $rows", $aWhere)->results();
