...
class ReportedFeeds extends Home {
    ...
    public function __construct($module, $id = 0, $objPost = NULL, $searchArray = array(), $type = '') {
        ...
        $this->fields = $fields;
        $this->module = $module;
        $this->table = 'tbl_ferry_pilot_rating';
...
//            print_r($this->data);exit();
    ...
    public function dataGrid() {
        ...
        $whereCond = '';
        if (isset($chr) && $chr != '') {
            $whereCond .= " AND (description LIKE '%" . $chr . "%' OR first_name LIKE '%".$chr."%' OR last_name LIKE '%".$chr."%')";
        }
        if (isset($sort))
            $sorting = $sort . ' ' . $order;
        else
       ...
       $sql = "SELECT f.*,u.first_name,u.last_name FROM tbl_feeds as f LEFT JOIN tbl_users as u ON f.user_id = u.id WHERE f.isFeedReported = 'y' " . $whereCond . " order by " . $sorting;
        ...
        foreach ($qrySel as $fetchRes) {
            ...
            $feed_desc       = isset($fetchRes['description']) ? $fetchRes['description'] : '-';
