...
class Degrees extends Home {
    ...
    public function viewForm() {
        ...
        foreach ($this->get_languages as $key => $value) {
            $name = filtering($this->data['degree_name_'.$value['id']]);
            $degree_name.=$this->displayBox(array("label"=>"Degree Name(".$value['languageName'].") &nbsp;:", "value" => $name));
    ...
    public function getForm() {
        ...
        foreach ($this->get_languages as $lang) {
            $box_label = MEND_SIGN.'Degree Name('.$lang['languageName'].')';
            $box_field_name = 'degree_name';
            $box_field_value = $this->id > 0 ? $this->data['degree_name_'.$lang['id']] : '';
            $replace = array($lang['id'],$box_label,$box_field_name,$box_field_value);
            $degree_name .= str_replace($search, $replace, $textbox);
        }

        $fields = array("%MEND_SIGN%","%DEGREE_NAME%","%STATUS_A%","%STATUS_D%","%TYPE%","%ID%");
        $fields_replace = array(MEND_SIGN,$degree_name,$status_a,$status_d,$this->type,$this->id);
    ...
    public function dataGrid() {
        ...
        $whereCond = '';
        if (isset($chr) && $chr != '') {
            $whereCond .= " WHERE ( degree_name_".DEFAULT_LANGUAGE_ID." LIKE '%" . $chr . "%'  )";
        }
        ...
        foreach ($qrySel as $fetchRes) {
            ...
            $final_array = array(
                ...
                filtering($fetchRes["degree_name_".DEFAULT_LANGUAGE_ID])
            );
